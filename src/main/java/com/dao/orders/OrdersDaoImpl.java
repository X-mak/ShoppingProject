...
public class OrdersDaoImpl extends BaseDao implements OrdersDao {
	...
	public Orders selectBuyerInfo(Orders o)throws SQLException{
		...
		ResultSet rs =  null;
		try {
			String sql = "SELECT * FROM buyerinfo WHERE u_act = ?";
			ps = conn.prepareStatement(sql);
	...
	public ArrayList<Orders> selectByBuyerStatus(String b_act,int o_status)throws SQLException{
		...
		ResultSet rs =  null;
		try {
			String sql = "SELECT * FROM orders WHERE u_act = ? AND o_status = ?";
			ps = conn.prepareStatement(sql);
	...
	public ArrayList<Orders> selectByAct(String b_act)throws SQLException{
		...
		ResultSet rs =  null;
		try {
			String sql = "SELECT * FROM orders WHERE u_act = ?";
			ps = conn.prepareStatement(sql);
			ps.setString(1, b_act);
			rs = ps.executeQuery();
			while(rs.next()) {
