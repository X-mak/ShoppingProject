...
public class MerchandiseDaoImpl extends BaseDao implements MerchandiseDao {
	...
	public Merchandise insertMerchan(Merchandise m)throws SQLException{
		...
		ResultSet rs = null;
		try {
			String sql = "INSERT INTO merchandise(m_name,m_intro,m_status) VALUES(?,?,1)";
			ps = conn.prepareStatement(sql,Statement.RETURN_GENERATED_KEYS);
			ps.setString(1, m.getM_name());
			ps.setString(2, m.getM_intro());
			ps.executeUpdate();
	...
	public Merchandise selectMerchan(Merchandise m)throws SQLException{
		...
		try {
			...
			rs = ps.executeQuery();
			if(rs.next()) {
				new_m = new Merchandise(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getInt(4));
			}
		}catch(Exception e) {
			e.printStackTrace();
		}finally {
	...
	public ArrayList<Merchandise> selectAllMerchanStatus()throws SQLException{
		...
		try {
			...
			rs = ps.executeQuery();
			while(rs.next()) {
				Merchandise m = new Merchandise(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getInt(4));
				l.add(m);
			}
		}catch(Exception e) {
			e.printStackTrace();
